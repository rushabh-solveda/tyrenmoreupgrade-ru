...
class Price extends \Magento\Framework\Data\Collection
{
        ...
        if ($printQuery instanceof Collection) {
            $collection = $printQuery;
                ...
                $collection->getSelect()
                    ->columns(
                        array(
                            'price' =>  new \Zend_Db_Expr('(SELECT GROUP_CONCAT(`price` SEPARATOR " & ") FROM `sales_order_item` WHERE `sales_order_item`.`order_id` = main_table.`entity_id` GROUP BY `sales_order_item`.`order_id`)')
                        )
                    );

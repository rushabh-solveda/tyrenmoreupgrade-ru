...
class Originalprice extends \Magento\Framework\Data\Collection
{
        ...
        if ($printQuery instanceof Collection) {
            ...
            $joined_tables = array_keys(
                ...
                $collection->getSelect()
                    ->columns(
                        array(
                            'originalprice' => new \Zend_Db_Expr('(SELECT GROUP_CONCAT(`original_price` SEPARATOR " & ") FROM `sales_order_item` WHERE `sales_order_item`.`order_id` = main_table.`entity_id` GROUP BY `sales_order_item`.`order_id`)')
                        )
                    );

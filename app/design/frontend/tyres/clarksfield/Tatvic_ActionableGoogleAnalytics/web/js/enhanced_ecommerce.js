			...
			this.tvc_checkout_steps = function(tvc_checkout_prod, tvc_login_flag){
				...
				if( tvc_login_flag &&
				   ...
				   performance.navigation.type == 0
					...
					jQuery(function () {
						jQuery('#checkout').on("change", '#customer-email', function(){
				...
				jQuery(function () {
					jQuery('#checkout').on("click", 'button[data-role="opc-continue"]', function(){
						tvc_obj.call_step(tvc_checkout_prod,3);
					});
					jQuery('#checkout').on("click", '.checkout', function(){
						tvc_obj.call_step(tvc_checkout_prod,4);
					});
                    // for mobile
                    jQuery('#checkout').on("click", 'div[data-role="opc-continue"]', function(){
                        tvc_obj.call_step(tvc_checkout_prod,3);
					});
                    jQuery('#checkout').on("click", '.mobile-checkout', function(){
                        tvc_obj.call_step(tvc_checkout_prod,4);
                    });
				});	
